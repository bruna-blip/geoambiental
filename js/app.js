...
async function setupUserHeader() {
  try {
    ...
    const displayName = profile?.nome || user.user_metadata?.name || user.email?.split('@')[0] || 'Usuário';
    if (nameEl) nameEl.textContent = displayName;
    const topbarSpan = document.querySelector('.topbar-user span');
    if (topbarSpan) topbarSpan.textContent = displayName;
    if (emailEl) emailEl.textContent = user.email || '';
    if (roleEl) roleEl.textContent = profile?.perfil || '';
    if (avatarEl && displayName) avatarEl.innerHTML = `<span>${displayName.charAt(0).toUpperCase()}</span>`;
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    // Inicialização leve para evitar travamento
      ...
      stats.innerHTML = `
        <div class="stat-card stat-green">
          ...
          <div class="stat-info">
            ...
            <div class="stat-label">Sistema iniciado</div>
            <div class="stat-sub">Clique nos menus para carregar os módulos</div>
          </div>
        </div>
      `;
    }
    ...
    if (lic) lic.innerHTML = '<div class="empty-list">Carregamento inicial leve ativado.</div>';

    const cond = document.getElementById('condicionantesUrgentes');
    if (cond) cond.innerHTML = '<div class="empty-list">Abra os módulos pelo menu lateral.</div>';
