...
function bindLogoutButton() {
  ...
  if (!logoutBtn || logoutBtn.dataset.boundLogout === 'true') return;
...
async function loadCurrentProfile(userId) {
  const queries = [
    () => tableQuery('profiles').select('*').eq('id', userId).maybeSingle(),
    () => tableQuery('profiles').select('*').eq('user_id', userId).maybeSingle(),
  ];
