...
const LOGIN_PAGE = GeoAmbientalConfig.loginPage || 'login.html';

if (!window.supabase?.createClient) {
  throw new Error('Biblioteca do Supabase não carregada. Inclua o CDN do supabase-js antes do app.js.');
}
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
  console.warn('Configure window.GEO_AMBIENTAL_CONFIG com supabaseUrl e supabaseAnonKey.');
}
...
const TABLE_CONFIG = {
  clientes: {
    ...
    searchColumns: ['razao_social', 'nome_fantasia', 'cnpj_cpf', 'setor', 'municipio', 'responsavel', 'email'],
  },
  licencas: {
    orderBy: { column: 'data_validade', ascending: true },
    searchColumns: ['numero', 'tipo', 'orgao_emissor', 'empreendimento', 'numero_processo', 'atividade', 'status'],
  },
  condicionantes: {
    orderBy: { column: 'prazo', ascending: true },
    searchColumns: ['numero', 'descricao', 'categoria', 'status', 'prioridade', 'responsavel'],
  },
  checklists: {
    orderBy: { column: 'titulo', ascending: true },
    searchColumns: ['titulo', 'categoria', 'status', 'observacoes'],
  },
  modelos_documentos: {
    orderBy: { column: 'titulo', ascending: true },
    searchColumns: ['titulo', 'categoria', 'descricao', 'tags', 'conteudo', 'status', 'versao'],
  },
};
...
function normalizeSearchQuery(table, search = '') {
  const text = escapeSupabaseLike(search);
  if (!text) return null;
  const cols = TABLE_CONFIG[table]?.searchColumns || [];
  if (!cols.length) return null;
  return cols.map((col) => `${col}.ilike.%${text}%`).join(',');
}

async function getCurrentProfile() {
  const { data: authData, error: authError } = await supabaseClient.auth.getUser();
  if (authError) throw authError;
  Auth.user = authData?.user || null;
...
async function requireAuth() {
  ...
  if (!profile) {
    toast('Seu usuário não possui perfil cadastrado.', 'error');
  ...
  if (profile.is_active === false) {
    toast('Seu acesso está inativo. Procure o administrador.', 'error');
    await supabaseClient.auth.signOut();
    redirectToLogin();
    return false;
  }
...
function applyUserInfo() {
  const name = Auth.profile?.full_name || Auth.user?.email || 'Usuário';
  const userLabel = document.querySelector('.topbar-user span');
  if (userLabel) userLabel.textContent = name;
}

function protectUiByRole() {
  if (userCanEdit()) return;
  document.querySelectorAll('.btn-primary, .btn-danger, .btn-outline').forEach((btn) => {
    if (btn.closest('.table-actions') || btn.closest('.toolbar-actions') || btn.closest('.modal-footer') || btn.classList.contains('btn-primary')) {
...
function formatDbError(error, fallback = 'Operação não concluída.') {
  const msg = error?.message || fallback;
  if (msg.includes('row-level security')) return 'Acesso negado para esta operação.';
  if (msg.includes('duplicate key')) return 'Já existe um registro com esse identificador.';
  return msg;
}
...
async function apiGet(table, params = {}) {
  ...
  const searchQuery = normalizeSearchQuery(table, search);
  if (searchQuery) query = query.or(searchQuery);

  const order = TABLE_CONFIG[table]?.orderBy;
  if (order?.column) query = query.order(order.column, { ascending: order.ascending ?? true, nullsFirst: false });
  query = query.range(start, end);

  const { data, error, count } = await query;
  if (error) throw new Error(formatDbError(error, 'Erro ao buscar dados.'));
...
async function apiGetById(table, id) {
  ...
  if (error) throw new Error(formatDbError(error, 'Erro ao carregar registro.'));
...
async function apiPost(table, data) {
  ...
  const { data: row, error } = await supabaseClient.from(table).insert(payload).select().single();
  if (error) throw new Error(formatDbError(error, 'Erro ao salvar.'));
  return row;
}
...
async function apiPut(table, id, data) {
  ...
  let query = supabaseClient.from(table).update(payload).select().single();
  if (isUUID(id)) query = query.eq('id', id);
  else query = query.eq('legacy_id', id);
  const { data: row, error } = await query;
  if (error) throw new Error(formatDbError(error, 'Erro ao atualizar.'));
  return row;
}
...
async function apiDelete(table, id) {
  ...
  if (error) throw new Error(formatDbError(error, 'Erro ao excluir.'));
}
...
function guardEditAction() {
  ...
  toast('Seu perfil possui acesso somente leitura.', 'warning');
  return false;
}

function guardDeleteAction() {
  if (userCanDelete()) return true;
  toast('Somente administradores podem excluir registros.', 'warning');
  return false;
}
...
function setupListeners() {
  ...
  document.querySelector('[onclick="openModal(\'modalCliente\')"]')?.addEventListener('click', (e) => { e.preventDefault(); if (!guardEditAction()) return; clearClienteForm(); document.getElementById('clienteId').value = ''; openModal('modalCliente'); });
  document.querySelector('[onclick="openModal(\'modalLicenca\')"]')?.addEventListener('click', async (e) => { e.preventDefault(); if (!guardEditAction()) return; await loadClientesSelect('licencaClienteId'); clearLicencaForm(); openModal('modalLicenca'); });
  document.querySelector('[onclick="openModal(\'modalCondicionante\')"]')?.addEventListener('click', async (e) => { e.preventDefault(); if (!guardEditAction()) return; await loadClientesSelect('condClienteId'); clearCondForm(); openModal('modalCondicionante'); });
  document.querySelector('[onclick="openModal(\'modalChecklist\')"]')?.addEventListener('click', async (e) => { e.preventDefault(); if (!guardEditAction()) return; await loadClientesSelect('checklistClienteId'); clearChecklistForm(); openModal('modalChecklist'); });
  document.querySelector('[onclick="openModal(\'modalModelo\')"]')?.addEventListener('click', (e) => { e.preventDefault(); if (!guardEditAction()) return; clearModeloForm(); document.getElementById('modeloId').value = ''; openModal('modalModelo'); });
  ...
  document.querySelector('[data-action=\"logout\"]')?.addEventListener('click', async (e) => { e.preventDefault(); await logout(); });
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    console.error(error);
    toast(formatDbError(error, 'Erro ao iniciar o sistema.'), 'error');
  }
});